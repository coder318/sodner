...
from sodner.models.tree import Tree
from sodner.models.tree_dep import TreeDep
from sodner.models.tree_feature import TreeFeature
...
class SodNER(Model):
    ...
    def __init__(self,
        ...
        if self.use_tree:
            self._tree = Tree.from_params(vocab=vocab, params=modules.pop("tree"))

        self.use_dep = use_dep
        if self.use_dep:
            self._dep_tree = TreeDep.from_params(vocab=vocab, params=modules.pop("dep_tree"))

        self.use_tree_feature = use_tree_feature
        if self.use_tree_feature:
            self._tf_layer = TreeFeature.from_params(vocab=vocab, params=modules.pop("tf_layer"))
